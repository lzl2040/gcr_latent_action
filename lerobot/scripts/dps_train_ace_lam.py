...
from lerobot.common.datasets.lerobot_dataset_for_ace_lam import MultiDatasetforDistTraining, extra_collate_fn
...
from lerobot.common.policies.ace.modeling_ace_with_lam import ACE_Latent_Model
...
def update_policy(
    ...
    model_engine(batch, model_engine.device)
    # loss, output_dict = model_engine(batch, model_engine.device)
...
def train(cfg: TrainPipelineConfig):
    ...
    ace_model = make_policy(
        cfg=cfg.policy2,
        device='cpu',
        ds_meta=dataset.meta,
        weight_pt_path=cfg.policy2.pretrained_path
    )
    policy = ACE_Latent_Model(lam_model=lam_policy, ace_model = ace_model)
    del lam_policy, ace_model
