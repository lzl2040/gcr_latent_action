...
from lerobot.common.constants import (IMAGENET_MEAN, IMAGENET_STD, CLIP_MEAN, CLIP_STD, 
...
def build_transform(is_train, input_size, pad2square=False, normalize_type='imagenet'):
    ...
    if is_train:  # use data augumentation
        transform = v2.Compose([
            ...
            v2.RandomChoice([v2.Lambda(jpeg_degrade_functions[quality]) for quality in qualities]),
            v2.Resize((input_size, input_size), interpolation=InterpolationMode.BICUBIC),
            v2.ToTensor(),
        ...
        if pad2square is False:  # now we use this transform function by default
            ...
            transform = v2.Compose([
                v2.Lambda(lambda img: img.convert('RGB') if img.mode != 'RGB' else img),
                v2.Lambda(lambda img: expand2square(img, tuple(int(x * 255) for x in MEAN))),
                v2.Resize((input_size, input_size), interpolation=InterpolationMode.BICUBIC),
                v2.ToTensor(),
                v2.Normalize(mean=MEAN, std=STD)
            ])
