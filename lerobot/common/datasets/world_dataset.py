...
from lerobot.common.datasets.video_utils_world import (
    VideoFrame,
...
from lerobot.common.datasets.video_transforms import ToTensorVideo, ResizeCrop
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.max_history_frame = max_history_frame
        print(f"Selected max frame num is: {self.max_frame}")
        self.repo_id = repo_id
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if len(self.meta.video_keys) > 0:
            ...
            primary_view_num = len(video_frames[primary_obs_key])
            # if primary_view_num < self.max_frame:
            #     need_skip = True
            #     print(f"Skip idx {idx} due to insufficient primary view frames: {primary_view_num} < {self.max_frame}")
...
class MultiDatasetforDistTraining(torch.utils.data.Dataset):
    def __init__(self, cfg, image_transforms, wrist_image_transforms = None, seed: int = 1000, 
        ...
        if self.cfg.dataset.processor is not None:
            # self.tokenizer = AutoTokenizer.from_pretrained(self.cfg.dataset.processor, add_eos_token=False, trust_remote_code=True, 
            #                                                use_fast=False)
            # self.transform = build_transform(is_train=True, input_size=256, pad2square=True)
            self.processor = InternVLProcessor.from_pretrained(self.cfg.dataset.processor)
            # self.processor.tokenizer.padding_side = "left"
            self.processor.video_processor.size["height"] = cfg.dataset.default_image_size
            self.processor.video_processor.size["width"] = cfg.dataset.default_image_size
            self.processor.image_processor.size["height"] = cfg.dataset.default_image_size
            self.processor.image_processor.size["width"] = cfg.dataset.default_image_size
            # internvl video processor config's image size = 448
            self.processor.image_seq_length = 256 // (448 // cfg.dataset.default_image_size) ** 2
            # add token
            new_tokens = cfg.policy.new_tokens
            self.processor.tokenizer.add_tokens(new_tokens)
            self.cp_sc_token_idx = [self.processor.tokenizer(new_tokens[0], add_special_tokens=False).input_ids[0]]
            self.cp_act_token_idx =  [self.processor.tokenizer(new_tokens[1], add_special_tokens=False).input_ids[0]]
            self.sc_token = new_tokens[0]
            self.act_token = new_tokens[1]
        ...
        for dataset_name in included_datasets:
            if dataset_name in vla2data_root.keys():
                ...
                dataset = LeRobotDataset(
                    ...
                    max_frame=cfg.policy.max_frame,
                    max_history_frame=cfg.policy.max_history_frame
                )
        ...
        if self.is_ft:
            ...
            for ds_id in range(len(self.datasets)):
                ...
                if num_samples >= len(dataset):
                    ...
                    # 有重复采样
                    sampled_indices = random.choices(data_indices, k=num_samples)
                    expand_ratio = num_samples / len(dataset)
                    new_episodes = int(dataset.num_episodes * expand_ratio)
        ...
        self.new_obs_image_keys = []
        for key in self.stats.keys():
            if key in all_new_obs_image_keys:
                self.new_obs_image_keys.append(key)
        img_feats = {}
        ...
        for new_key in self.new_obs_image_keys:
    ...
    def pad_vector(self, vector, new_dim):
        ...
        current_dim = shape[-1]
        shape[-1] = new_dim
        new_vector = torch.ones(*shape, dtype=vector.dtype, device=vector.device) * -1
    ...
    def __getitem__(self, index):
        ...
        # print(data_dict["action"].shape, data_dict["video_tensor"].shape) # 30, 29
        return data_dict
    ...
    def norm_data_with_quantile(self, item):
        ...
        if "agi" in item['dataset_name']:
            ...
            state_q01[state_start_dim:state_end_dim] = self.stats["observation.state"]["q01"][state_start_dim:state_end_dim]
            state_q99[state_start_dim:state_end_dim] = self.stats["observation.state"]["q09"][state_start_dim:state_end_dim]
            action_q01[action_start_dim:action_end_dim] = self.stats["action"]["q01"][action_start_dim:action_end_dim]
            action_q99[action_start_dim:action_end_dim] = self.stats["action"]["q09"][action_start_dim:action_end_dim]
            denom = action_q99 - action_q01
            denom = torch.where(
                denom == 0, torch.tensor(1e-8), denom
            )
            ...
            # print(action_q01[:14], action_q99[:14])

            denom = action_q99 - action_q01
            denom = torch.where(
                denom == 0, torch.tensor(1e-8), denom
            )
            # print(torch.min(item["action"]), action_q01[:7])
            item["action"] = 2.0 * (item["action"] - action_q01) / denom - 1.0
            # print(item["action"][:, :14])
            action_mask[:8] = 1
            # item["action"] = (item["action"] + 1.0) * denom / 2.0 + action_q01
        item["action_mask"] = action_mask
        # print(torch.max(item["action"][:, :7]))
        return item
    ...
    def _prepare_intern_vl_data(self, item):
        vision, first_image, last_image = self._prepare_images(item)
        ...
        # video = vision["video"]
        history_video = vision["history_video"]
        ...
        # if self.train:
        # add answer
        compress_sc_replace = ", ".join([self.sc_token for i in range(self.cfg.policy.num_sc_token)])
        compress_act_replace = ", ".join([self.act_token for i in range(self.cfg.policy.num_action_token)])
        ...
        # future_video = vision["video"]
        # if len(future_video) > 1:
        #     furture_images = future_video[1:]  # 去掉第一帧
        # else:
        furture_images = vision["video"]
        ...
        # print(video_tensor.shape, pixel_values.shape)
        return_dict = {
            "input_ids": input_ids,
            "labels": labels,
    ...
    def _prepare_images(self, item):
        ...
        # 只取其中一个视角的
        present_img_keys = [present_img_keys[0]]
        ...
        for key in present_img_keys:
            ...
            for i in range(len(vision['video'])):
                vision["video"][i] = vision["video"][i].resize((256, 256))
            
        vision["history_video"] = history_frames
        for i in range(len(vision['history_video'])):
            vision["history_video"][i] = vision["history_video"][i].resize((256, 256))
