...
class LatentActionProjection(nn.Module):
    def __init__(self, config: PaliGemmaWithExpertConfig):
        super().__init__()
        config = config.paligemma_config
        self.q_proj = nn.Linear(config.text_config.hidden_size, 
                                config.text_config.num_attention_heads * config.text_config.head_dim, 
                                )
        self.k_proj = nn.Linear(config.text_config.hidden_size, 
                                config.text_config.num_key_value_heads * config.text_config.head_dim, 
                                )
        self.v_proj = nn.Linear(config.text_config.hidden_size, 
                                config.text_config.num_key_value_heads * config.text_config.head_dim)
