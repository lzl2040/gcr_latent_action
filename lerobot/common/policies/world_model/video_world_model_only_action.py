...
from lerobot.common.policies.world_model.modified_sana_transformer import Modified_SanaVideoTransformerBlock_Action, Modified_SanaModulatedNorm, Modified_WanRotaryPosEmbed
...
class VideoWorldModel(nn.Module):
    def __init__(self, config):
        ...
        # https://github.com/NVlabs/Sana/blob/main/configs/sana_video_config/Sana_2000M_480px_AdamW_fsdp_chunk.yaml#L97
    ...
    def prepare_modules(self):
        ...
        for i in range(self.transformer.config.num_layers):
            self.transformer.transformer_blocks[i] = Modified_SanaVideoTransformerBlock_Action(
                **block_kwargs)
    ...
    def prepare_noise_action(self, actions, sigmas, states, device="cuda"):
    ...
    def forward(self, img_embeds, sc_embeds, act_embeds, task_info_dict, target_imgs, actions, states, train_step):
        ...
        noisy_action_input, action_target, action_noise = self.prepare_noise_action(actions, sigmas[:, None, None], 
                                                                                       states, device)
        ...
        loss = {}
        loss["video_loss"] = torch.tensor(0.0, device=device)
