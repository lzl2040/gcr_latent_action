"""Standalone test script for Action Chunk Encoder (ACE) module."""

import sys
import os

# Add the ace directory to path
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

import torch
from lerobot.common.policies.ace.configuration_robo_clip import ACEConfig
from modeling_ace import ActionChunkEncoder, create_ace_model
...
def test_basic_forward():
    ...
    config = ACEConfig(
        ...
        chunk_size=16,
        group_size=4,
        hidden_dim=256,  # Must be divisible by num_attention_heads
...
def test_grouping_mechanism():
    ...
    batch_size = 1
    actions = torch.arange(config.chunk_size).float().unsqueeze(-1).repeat(1, 1, config.action_dim)
    actions = actions.unsqueeze(0)  # Already has correct shape (1, 16, 7)
...
def test_different_configs():
    ...
    configs = [
        {"action_dim": 7, "chunk_size": 8, "group_size": 2, "hidden_dim": 128, "num_attention_heads": 8},
        {"action_dim": 14, "chunk_size": 32, "group_size": 8, "hidden_dim": 256, "num_attention_heads": 8},
        {"action_dim": 32, "chunk_size": 16, "group_size": 4, "hidden_dim": 192, "num_attention_heads": 8},  # No padding needed
    ]
