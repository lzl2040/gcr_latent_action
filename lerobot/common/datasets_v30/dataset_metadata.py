...
from lerobot.common.datasets_v30.compute_stats import aggregate_stats
from lerobot.common.datasets_v30.feature_utils import _validate_feature_names, create_empty_dataset_info
...
from lerobot.common.datasets_v30.video_utils import get_video_info
...
class LeRobotDatasetMetadata:
    ...
    def load_metadata(self):
        ...
        if "observations.ee_ort6d_pos" in self.stats.keys():
            ...
            if "observation.ee_ort6d_pos" in self.stats.keys():
                ...
                if "observations.ee_6d_pos" in self.stats.keys():
                    self.stats["observation.state"] = self.stats["observations.ee_6d_pos"]
        ...
        if "action.ee_ort6d_pos" in self.stats.keys():
            ...
            if "action.ee_6d_pos" in self.stats.keys():
                    self.stats["action"] = self.stats["action.ee_6d_pos"]
    ...
    def camera_keys(self) -> list[str]:
        ...
        return [key for key, ft in self.features.items() if ft["dtype"] in ["video"]]
    ...
    def create_with_stats_feats(
        ...
        obj = cls.__new__(cls)
        obj.stats = stats
        obj.info = create_empty_dataset_info(CODEBASE_VERSION, fps, robot_type=robot_type, features=features, use_videos=use_videos)
        return obj
