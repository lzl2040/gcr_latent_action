...
deepspeed --master_port=${MASTER_PORT} "$TRAIN_ENTRYPOINT" \
    ...
    --num_workers=12 \
    --output_dir="${OUTPUT_DIR:-robo_contrast}" \
    --steps=${STEPS:-60_0000} \
    --log_freq=20 \
    --eval_freq=250 \
    ...
    --policy.vision_tuning_mode="${VISION_TUNING_MODE:-lora}" \
    --policy.vision_lora_rank="${VISION_LORA_RANK:-16}" \
    --policy.vision_lora_alpha="${VISION_LORA_ALPHA:-16}" \
    --policy.vision_lora_dropout="${VISION_LORA_DROPOUT:-0.0}" \
    --policy.vision_lora_layers="${VISION_LORA_LAYERS:-4}" \
    --policy.vision_lr_scale="${VISION_LR_SCALE:-0.1}" \
