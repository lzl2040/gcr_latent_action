...
| `lerobot/common/datasets/contrastive_eval.py` | 固定评测集，训练内指标噪声太大无法比较运行 |
...
| 数据集 | state 漂移(中位) | 触觉图像 \|t1−t0\| | 触觉信号逐帧 std |
| --- | --- | --- | --- |
| `ftp_1_RH20TCfg5Franka` | 0.580 | — | 0.135 |
| `ftp_1_sharpa_split_0` | 0.308 | 0.493（max 7.60） | — |
| `ftp_1_VisuoTactile_D-WHEEL_split_0` | 0.326 | 2.741 | — |

注意三个数据集是互补的：RH20T 只有触觉**信号**，另外两个只有触觉**图像**。sharpa 的中位数低
而最大值高，正说明大部分窗口无接触变化、少数窗口有——这正是我们希望模型去分辨的信号。
...
## 5. 固定评测集（`contrastive_eval.py`）

训练日志里的 `acc`（batch 内检索准确率）**不能用来比较两次运行**。它的 batch 是随机抽的，而
"哪些数据集、哪些 episode 恰好凑在一起"对这个数字的影响远大于模型质量。实测：同一份代码、
同一个 seed 跑两遍，在 800–1060 步窗口上平均绝对差 **0.055**——比我们试图测量的任何代码改动
的效果都大。dataloader worker 的交错和视频解码都不是确定性的，所以同 seed 并不产生同 batch。

`contrastive_eval.py` 的做法是把帧固定死：启动时确定性地抽一批 `(dataset_idx, frame_idx)`
batch，之后每次评测、每次运行都打分**同样这些帧**，剩下变化的只有模型。
...
| --- | --- | --- |
| `mixture` | 训练权重 | 主指标 |
| `tactile` | 只从有触觉的数据集采 | 触觉专用指标 |

`tactile` split 是必需的：`debug_research_data` 里触觉只占 **2.7%**，256 行的 batch 里平均
只有 7 行带触觉，主指标对触觉侧的任何改动几乎是结构性失明的。这个 split 里 100% 是触觉样本，
每次评测覆盖 2048 行。

两个实现细节：

- **准确率在单个 batch 内计算，不跨 all-gather。** 这样无论用几张卡，任务都是固定的 N 路
  检索，不同卡数的运行之间数字仍可比。各 rank 按 `batches[rank::world_size]` 分片，只在最后
  做一次 all-reduce 汇总。
- **`model.eval()` 关掉了 modality dropout**，所以评测是确定性的。

一个必须说清楚的局限：这些帧抽自训练 mixture，**不是按 episode 划分出来的留出集**。在这些
运行达到的规模上（36M 帧跑约 0.6 epoch），某一帧被训练过的概率是百分之几，所以实际上等价于
留出；但它不是构造上留出的，跑几个 epoch 之后这个说法就不成立了。
...
**触觉图像的两帧被折进同一个 token，而不是各占一个。** token 由 `[feat_t, feat_t1 - feat_t]`
投影而成，差分项才是重点：模型要对齐的是窗口内的变化，不是 `t` 时刻的绝对读数。折进一个
token 是为了不让触觉相机的 token 数翻倍去挤压动作块。触觉信号则不同——它读满了 16 帧，和
state / action 一样按 `group_size` 分组。
