...
16 卡、cached latents、stage-3 混合（batch 取各设备放得下的最大值），三种口径并列：

| scope | 设备 | batch/GPU | 显存 | step | clips/s | `frames`（你的） | `windows` | `coverage` |
|---|---|---|---|---|---|---|---|---|
| `gen_only` | RTX A6000 | 64 | 35.1 G DDP | 4079 ms | 251 | 33.2 d | 1.0 d | 3.7 d |
| `gen_only` | A100-40GB | 64 | 35.1 G DDP | 1168 ms | 877 | **9.5 d** | 7.1 h | 1.1 d |
| `gen_only` | A100-80GB | 128 | 51.8 G DDP | 2178 ms | 940 | **8.9 d** | 6.6 h | 23.6 h |
| `gen_only` | B200 | 128 | 51.8 G DDP | 508 ms | 4030 | **2.1 d** | 1.6 h | 5.5 h |
| `freeze_vision` | RTX A6000 | 32 | 20.6 G ZeRO-2 | 3184 ms | 161 | 51.8 d | 1.6 d | 5.8 d |
| `freeze_vision` | A100-40GB | 32 | 20.6 G **ZeRO-2** | 1038 ms | 493 | **16.9 d** | 12.7 h | 1.9 d |
| `freeze_vision` | A100-40GB | 64 | 28.9 G **ZeRO-2** | 1702 ms | 602 | **13.9 d** | 10.4 h | 1.5 d |
| `freeze_vision` | A100-80GB | 128 | 73.2 G DDP | 3030 ms | 676 | **12.3 d** | 9.2 h | 1.4 d |
| `freeze_vision` | B200 | 128 | 73.2 G DDP | 739 ms | 2772 | **3.0 d** | 2.3 h | 8.0 h |

在线跑 VAE（不缓存 latent）大约多 40–50%。上表已假设 latent 离线缓存好。
...
要发 6 次 read**。按 200 ms/clip 估：

| 设备 | batch/GPU | clips/s/GPU | 需要 worker 数 | 16 卡合计带宽 | 16 卡合计 IOPS |
|---|---|---|---|---|---|
| RTX A6000 | 32 | 12 | 2 | 126 MiB/s | 1,142 |
| A100 | 32 | 43 | **9** | 457 MiB/s | 4,144 |
| B200 | 32 | 190 | **38** | 2.0 GiB/s | 18,265 |

**卡脖子的是延迟和 IOPS，不是带宽。** 457 MiB/s 对任何云存储都不算什么，但每秒 4000 次
随机小读、每次几百 KiB，在 blobfuse 上很容易打到限流。两个应对：

1. **离线缓存 VAE latent**（1.66 TB）。loader 从"seek 进 mp4 解码"变成"顺序读 72 KiB 张量"，
   read 次数从 6 降到 1，且完全没有解码。在网络挂载上这个收益比本地大得多。
2. **worker 数按延迟配**：`workers ≈ clips/s × 延迟`。780 ms（机械盘）要 ~24 个，
   200 ms（挂载）要 ~6–9 个，37 ms（热/SSD）只要 1.2 个。当前默认 12 个 worker
   在挂载盘上刚好够 A100，B200 一定不够。
