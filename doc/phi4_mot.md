...
| `gen_only` | A100-40G | 64 | 100G Eth | 原视频 | 1789 | 13 | 1067 | 1802 ms | 计算 | **14.7 d** |
| `gen_only` | B200 | 128 | 8×200G IB | latent | 476 | 2 | 640 | 640 ms | **I/O** | **2.6 d** |
| `gen_only` | B200 | 128 | 8×200G IB | 原视频 | 794 | 2 | 2133 | 2133 ms | **I/O** | 8.7 d |
| `freeze_vision` | A100-40G | 32 | 8×200G IB | latent | 741 | 6 | 160 | 748 ms | 计算 | **12.2 d** |
| `freeze_vision` | A100-40G | 32 | 8×200G IB | 原视频 | 1092 | 6 | 533 | 1099 ms | 计算 | 17.9 d |
| `freeze_vision` | A100-40G | 32 | 100G Eth | latent | 741 | **385** | 160 | 1127 ms | 通信 | **18.3 d** |
...
| `freeze_vision` | B200 | 128 | 8×200G IB | 原视频 | 939 | 6 | 2133 | 2133 ms | **I/O** | 8.7 d |
...
它的算力（读原视频含在线 VAE 也才 939 ms）根本用不上，因为 12 个 worker 读原始视频
...
|---|---|---|---|---|---|---|---|---|
| RTX A6000 | `gen_only` | 64 | 6491 (3943+2548) | 1067 | 6493 | 计算 | 158.5 d | 158.8 d |
| A100-40GB | `gen_only` | 64 | 1789 (1087+702) | 1067 | 1791 | 计算 | **43.7 d** | 44.0 d |
| A100-80GB | `gen_only` | 128 | 3501 (2096+1405) | 2133 | 3503 | 计算 | **42.8 d** | 42.9 d |
| B200 | `gen_only` | 128 | 794 (476+318) | 2133 | 2133 | **I/O** | **26.0 d** | 26.0 d |
| RTX A6000 | `freeze_vision` | 32 | 3963 (2689+1274) | 533 | 3969 | 计算 | 193.8 d | 195.8 d |
| A100-40GB | `freeze_vision` | 32 | 1092 (741+351) | 533 | 1099 | 计算 | **53.6 d** | 59.8 d |
| A100-80GB | `freeze_vision` | 128 | 4137 (2732+1405) | 2133 | 4143 | 计算 | **50.6 d** | 51.1 d |
| B200 | `freeze_vision` | 128 | 939 (620+318) | 2133 | 2133 | **I/O** | **26.0 d** | 26.0 d |

三个读得出来的结论：

**a) 26.0 天是任何显卡的地板。** 60 clips/s/GPU × 16 = 960 clips/s，
2.16e9 / 960 = 2.25e6 s = 26.0 d。B200 两个 scope 都精确落在这个数上，因为它的算力
（794 / 939 ms）比 loader（2133 ms）快一倍多，**多出来的算力全部浪费**。
换句话说读原视频时 B200 和 A100-80GB 只差 1.6×，而不是纸面上的 4×。

**b) 网络几乎不重要。** 原视频把算力抬得很高，all-reduce 藏得进反向，
8×200G IB 和 100G 以太网只差 0.2–2%。唯一例外是 `freeze_vision` + A100-40GB
（53.6 → 59.8 d，+12%），因为那一档 batch 只有 32，step 最短、最藏不住通信。

**c) 在线 VAE 让 A100-40GB 输给 A100-80GB 的优势消失。** 两者算力相同，
80GB 版靠 batch 128 摊薄固定开销本该更快，但 batch 128 的 VAE 要 1405 ms，
把优势吃掉大半——`gen_only` 只快 2%，`freeze_vision` 反而在 40GB 上更好看
（53.6 vs 50.6 d 是 80GB 略胜，但差距只有 6%）。

对比预提 latent（§7.3 表）：3 万小时下 A100-40GB `freeze_vision`
36.5 d → **53.6 d（+47%）**，B200 7.8 d → **26.0 d（+233%）**。
